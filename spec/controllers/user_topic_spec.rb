...
RSpec.describe Sinatra::App:: TopicController do
  ...
  let(:topic2) {FactoryBot.create :topic, title: 'Testing topic', description: 'The long description on ruby' }
  ...
  context 'with valid credentials' do
    ...
    it 'sends email, when a user enrolls for a topic ' do
      ...
      message = "<h2>Welcome <strong>#{user.username.capitalize}</strong></h2>"
    ...
    it 'user un-follows  a topic ' do
      ...
      expect(last_response.body).to include('Enrolled Courses')
      expect(last_response.body)
        .to include("<h2>Welcome <strong>#{user.username.capitalize}</strong></h2>")
    end
  end
