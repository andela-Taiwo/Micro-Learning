  ...
  describe "Admin access topic" do
    ...
    context "with valid credentials" do
      ...
      it "add new the topic" do
        post "/admin/topic", topic: attributes_for(:topic)
        follow_redirect!
        expect(last_response).to be_ok
      ...
      it "edit the topic description " do
        ...
        patch "/admin/topic/#{topic.id}",
              attributes_for(:topic, description: topic_description)
        follow_redirect!
        expect(last_response).to be_ok
      ...
      it "edit the resource title" do
        patch "/admin/topic/#{topic.id}",
              attributes_for(:topic, title: " The new title ")
        follow_redirect!
        expect(last_response).to be_ok
        expect(last_response.body).to include("Successfully updated the topic")
    ...
    context "with invalid data set" do
      it "does not add new the topic when title is blank" do
        post "/admin/topic", topic: attributes_for(:topic, title: " ")
        follow_redirect!
        expect(last_response).to be_ok
        expect(last_response.body).to include("title is too short (minimum is 6 characters")
      end

      it "does not add new the topic when description is blank" do
        post "/admin/topic", topic: attributes_for(:topic, description: " ")
        follow_redirect!
        expect(last_response).to be_ok
      ...
      it "when blank title is provided, topic title should remain unchange " do
        patch "/admin/topic/#{topic.id}", attributes_for(:topic, title: " ")
        follow_redirect!
        expect(last_response).to be_ok
